---
title: "2025 State of the Map US - MapSwipe Today and Its Future in OSM"
description: "A talk about MapSwipe and it's future in OSM"
layout: "single"
draft: false
date: 2025-07-03
tags: ["Mapping", "OpenStreetMap", "Open Data", "MapSwipe"]
coverCaption: "Presenting on a MapSwipe past use case. Photo by Juan Carlos Melo Luna. 😊"
coverAlt: "Nicole Siggins stands at a podium, presenting a past MapSwipe use case."
featureAlt: "Nicole Siggins stands at a podium, presenting a past MapSwipe use case."
thumbnailAlt: "Presenting on a MapSwipe past use case. Photo by Juan Carlos Melo Luna. 😊."
---

I presented my talk, “MapSwipe Today and Its Future in OSM” on Saturday, June 21th at 13:45 local time in the Metcalf Large Ballroom at Boston University’s George Sherman Union.

{{< button href="https://www.youtube.com/watch?v=n4_G51fgT-8" target="_blank" >}}
Watch the video on YouTube
{{< /button >}}

### Talk synopsis

MapSwipe is a mobile and web app that enables crowdsourced data collection for OpenStreetMap (OSM). Today, MapSwipe helps prioritize mapping efforts by identifying areas needing updates. This talk will explore future possibilities for exploring expanding its role, including potentially allowing direct edits to OSM and strengthening ties with the OSM community for more seamless contributions. Feedback on these ideas is welcome!

{{< button href="https://docs.google.com/presentation/d/1L2LZmPlgjGFVl_ycwHurDgaD88L6T0XSN1b7V2oHLKk/edit?slide=id.p#slide=id.p" target="_blank" >}}
View the presentation slides
{{< /button >}}


Most importantly, I'd love to hear ideas and feedback about the idea of MapSwipe becoming it's own OSM editor (and including potential machine learning into the mix) from others in the OpenStreetMap community. There is an OSM diary entry where you can add your thoughts.

{{< button href="https://www.openstreetmap.org/user/Nicolelaine/diary/406967" target="_blank" >}}
Join the discussion
{{< /button >}}